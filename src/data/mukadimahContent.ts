export const mukadimahContent = {
  hook: "Pernah merasa bingung harus melangkah ke mana?",
  sub: "Itu wajar, dan banyak teman seusiamu merasakannya. Dunia terasa luas dan penuh pilihan, kadang malah membuat ragu.",
  paragraphs: [
    "Sulu hadir bukan untuk memberi jawaban instan, apalagi vonis tentang \"kamu cocoknya jadi apa\". Anggap saja ini nyala kecil di awal langkahmu: membantumu melihat dunia apa adanya, peluang dan tantangannya, lalu melihat ke dalam dirimu. Bukan untuk menakuti, melainkan supaya kamu melangkah dengan mata terbuka.",
    "Kamu juga tidak datang dengan tangan kosong. Allah sudah menitipkan sesuatu dalam dirimu, entah minat, karakter, atau kemampuan, yang menunggu dikenali dan diasah. Tugas kita bukan mencetakmu menjadi orang lain, tapi menyalakan apa yang sudah ada, lalu mengarahkannya ke tempat yang tepat.",
    "Sebab yang bertahan bukan yang paling ramai, tapi yang paling bermanfaat. Kamu tidak harus menjadi yang paling hebat di ibu kota; cukup menjadi cahaya di tempatmu sendiri, lalu menyalakan orang-orang di sekitarmu. Mulailah dari yang kecil, tumbuh dengan tenang, penuh syukur. Mari kita mulai.",
  ],
  ctaLabel: "Mari kita mulai",
  ctaTo: "/mukadimah",
  depthLabel: "Lihat dasar Qur'annya",
  dalil: [
    {
      ref: "QS Al-An'am: 122",
      arab: "اَوَمَنْ كَانَ مَيْتًا فَاَحْيَيْنٰهُ وَجَعَلْنَا لَهٗ نُوْرًا يَّمْشِيْ بِهٖ فِى النَّاسِ",
      terjemah:
        "Apakah orang yang sudah mati lalu Kami hidupkan dan Kami beri dia cahaya yang membuatnya dapat berjalan di tengah-tengah orang banyak…",
    },
    {
      ref: "QS Ar-Ra'd: 17",
      arab: "فَأَمَّا ٱلزَّبَدُ فَيَذۡهَبُ جُفَآءٗۖ وَأَمَّا مَا يَنفَعُ ٱلنَّاسَ فَيَمۡكُثُ فِى ٱلۡأَرۡضِ",
      terjemah:
        "Adapun buih, akan hilang sebagai sesuatu yang tidak berguna; tetapi yang memberi manfaat kepada manusia, akan tetap di bumi.",
    },
    {
      ref: "QS Ibrahim: 7",
      arab: "لَىِٕنْ شَكَرْتُمْ لَاَزِيْدَنَّكُمْ",
      terjemah:
        "Sesungguhnya jika kamu bersyukur, niscaya Aku akan menambah (nikmat) kepadamu…",
    },
    {
      ref: "QS An-Nahl: 97",
      arab: "مَنْ عَمِلَ صَالِحًا مِّنْ ذَكَرٍ اَوْ اُنْثٰى وَهُوَ مُؤْمِنٌ فَلَنُحْيِيَنَّهٗ حَيٰوةً طَيِّبَةً",
      terjemah:
        "Siapa yang mengerjakan kebajikan, baik laki-laki maupun perempuan, dalam keadaan beriman, pasti akan Kami berikan kepadanya kehidupan yang baik…",
    },
  ],
};
