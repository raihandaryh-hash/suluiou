...
export default function Ringkasan() {
  const { user } = useAuth();
  const [surat, setSurat] = useState(() => compileSurat());
  const [filter, setFilter] = useState<CatFilter>("all");

  // Hydrate journaling dari Supabase untuk user login: sembuhkan "bagian hilang" saat
  // origin/sesi berbeda (localStorage kosong). Isi HANYA bila LS kosong (tak menimpa data lokal).
  // JANGAN sentuh kenali_dirimu_sessions (asesmen terkunci).
  useEffect(() => {
    if (!user) return;
    let cancelled = false;
    (async () => {
      try {
        const [p2a, p2b, p3, p4, p5] = await Promise.all([
        ...
        const fill = (lsKey: string, row: { data?: unknown } | null) => {
          try {
        ...
        const changed = [
          fill("sulu_phase2a", p2a.data as { data?: unknown } | null),
          fill("sulu_phase2b", p2b.data as { data?: unknown } | null),
          fill("jalan_bakti_v1", p3.data as { data?: unknown } | null),
          fill("sulu_phase4_sintesis", p4.data as { data?: unknown } | null),
          fill("sulu_phase5_action_plan", p5.data as { data?: unknown } | null),
        ].some(Boolean);
        if (changed && !cancelled) setSurat(compileSurat());
      } catch {
        /* noop */
      }
    })();
